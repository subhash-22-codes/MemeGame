...
def handle_create_room(data):
    ...
    try:
        ...
        if not host:
            ...
            return
        existing_room = rooms_collection.find_one({"players.id": host.get("id"), "gamePhase": "lobby"})
        if existing_room:
            emit("error", {"error": "You already have an active room", "code": "ROOM_ALREADY_EXISTS"}, to=sid)
            return
    ...
    try:
        ...
        if existing_idx is not None:
            ...
            if len(room.get("players", [])) >= MAX_PLAYERS:
                emit(
                    "error",
                    {"error": "Room is full (max 8 players)", "code": "ROOM_FULL"},
                    to=sid
                )
                return
...
def handle_score_meme(data):
    ...
    try:
        ...
        total_rounds = room.get("totalRounds", 3) 
        submissions = room.get("submissions", [])
